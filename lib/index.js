...
Wig.prototype = {
  ...
  renderAll: function() {
    // build params
    var p, template, paramHash, templateHash, outputPath;
    for (var fPath in this.params) {
      //skip if directory
      if (fPath.substr(-1) !== '/') {
        p = this.buildParams(fPath);
        outputPath = fPath + (p._ext ? '.' + p._ext : '');
        if (p._template) {
          var tmplFile = false;
        ...
        //get template
        template = this.renderer.getTemplate(tmplFile.file);

        // check if updated
        paramHash = hash(p);
        templateHash = hash(template);
        if (paramHash !== this.paramHashes[fPath] || templateHash !== this.templateHashes[fPath]) {
          var fOutput = this.renderer.render(template, p);
          if (p._encoding) {
          ...
          this.paramHashes[fPath] = paramHash;
          this.templateHashes[fPath] = templateHash;
        }
      }
    }
  },
  ...
  readParams: function(fPath, fStat) {
    var params;
    if (fStat.mtime > (this.dataLastModified[fPath] || 0)) {
      // update data
      if (fPath.substr(-5) === '.json' || fPath.substr(-3) === '.js') {
        ...
        this.dataLastModified[fPath] = fStat.mtime;
      }
      this.dataCaches[fPath] = params;
    }
    return this.dataCaches[fPath];
  },
