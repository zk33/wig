...
Wig.prototype = {
  ...
  renderAll: function() {
    var baseParams, allPageParams, pageParams;
    //build all page list
    var allPages = [];
    ...
    allPages.forEach(function(item) {
      ...
      item.params._all_pages = dirPages['/'];
      //render
      ...
      if (item.params['_encoding']) {
        ...
        fContent = iconv.encode(fContent, item.params['_encoding']);
      }

      //if content changed : write file
      if (!this.contentHashes[item.path]) {
        this.contentHashes[item.path] = hash(this.getPageContent(item.path));
      }
      var newContentHash = hash(fContent);
      if (this.contentHashes[item.path] !== newContentHash) {
        this.savePage(item.path, fContent);
        //output to console
        if (this.options.verbose) {
          console.log(item.path + "\t <- \t" + fOutput.templatePath);
        }
        this.contentHashes[item.path] = newContentHash;
      }
    }, this);
  },
